...
function luudulieu(){
	...
	if($id != '')
	{
		...
		$d->setWhere('id',$id);
		if($d->update($data)){
			$d->redirect("index.php?p=slider&a=man&page=".@$_REQUEST['page']."");
		}
		else{
			 $d->transfer("Cập nhật dữ liệu bị lỗi", "index.php?p=slider&a=man");
		}
	}
	else
	{
		...
		if($d->insert($data))
		{
			$d->redirect("index.php?p=slider&a=man");
		}
		else{
			 $d->transfer("Thêm dữ liệu bị lỗi!", "index.php?p=slider&a=man");
		}
	}
}
...
function xoadulieu(){
	...
	if(isset($_GET['id'])){
		...
		if($d->delete()){
			...
			$d->transfer("Xóa dữ liệu bị lỗi", "index.php?p=slider&a=man");
		}
	}else $d->transfer("Không nhận được dữ liệu", "index.php?p=slider&a=man");
}
...
function xoadulieu_mang(){
	...
	if(isset($_POST['chk_child'])){
		...
		else $d->transfer("Xóa dữ liệu bị lỗi", "index.php?p=slider&a=man");
	}else $d->redirect("index.php?p=slider&a=man&page=".@$_REQUEST['page']."");
}
?>