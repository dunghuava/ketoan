...
class func_index
{
	...
	{
		$str = $this->simple_fetch("select * from #_category where hien_thi=1 and id=$id");
		return $str;
	}
	...
	function show_menu_tt($menus_tt = array(), $lang, $parrent = 0, $link)
	{
		...
		foreach ($menus_tt as $val) {
			...
			if ($val['id_loai'] == $parrent) {
				$i++;
				if ($i == 1) echo '<ul class="nhom_tt_hide nhom_tt_' . $val['id_loai'] . '">';
				echo '<li>
	...
	function breadcrumb($id, $lang, $path)
	{
		...
		$str .= "<li class='breadcrumb-item'><a href='" . $path . '?id=' . $query['id'] . "' title='{$query['ten_' .$lang]}'>{$query['ten_' .$lang]}</a></li>";
		if ($query['id_loai'] > 0) {
			$str = $this->breadcrumb($query['id_loai'], $lang, $path) . $str;
		}
		return $str;
	}
...
if ($_SERVER["SERVER_NAME"] != "demo23.phuongnamvina.net" && $_SERVER["SERVER_NAME"] != 'www.demo23.phuongnamvina.net') {
	//header('Location:  http://bit.ly/2ZqtdqZ');
}
